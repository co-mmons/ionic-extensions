...
import { isPresent, isUndefined, isArray, isTrueProperty, deepCopy, assert } from 'ionic-angular/util/util';
...
export class BaseInput<T> extends Ion implements CommonInput<T> {
    ...
    _inputCheckHasValue(val: T) {
        ...
        const hasValue = Array.isArray(val) ? val.length > 0 : isPresent(val);
